...
const LEGACY_JWT_SECRETS = ['your_jwt_secret', 'your_jwt_secret_key', 'BANNU9'];

const verifyJwtWithFallbacks = (token) => {
  const secretsToTry = [JWT_SECRET, ...LEGACY_JWT_SECRETS.filter((secret) => secret !== JWT_SECRET)];
  let lastError;
...
const authMiddleware = (req, res, next) => {
    ...
    if (!token || token === 'null' || token === 'undefined') {
        return res.status(401).json({ message: 'Authentication token missing. Please login again.' });
    }

    try {
        // 2. Verify token: MUST use the same key as generateToken
        const decoded = verifyJwtWithFallbacks(token);
...
    // Generate JWT token
    const token = jwt.sign({ id: client._id }, JWT_SECRET, { expiresIn: "7d" });
